# /rpi:fix Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Add a `/rpi:fix` command that runs Luna → Mestre → Forge in one step for bugfixes, creating compact artifacts (REQUEST.md, PLAN.md, IMPLEMENT.md) with max 3 tasks.

**Architecture:** Single command file (`commands/rpi/fix.md`) that orchestrates three agents sequentially with bugfix-optimized prompts. Reuses existing agents (Luna, Mestre, Forge) without modifications. Creates a reduced directory structure (no `research/`, no `delta/`).

**Tech Stack:** Markdown command files, Node.js test runner (node:test)

---

### Task 1: Add "fix" to EXPECTED_COMMANDS in tests

**Files:**
- Modify: `test/commands.test.js:10-28`

**Step 1: Add "fix" to EXPECTED_COMMANDS array**

In `test/commands.test.js`, add `"fix"` to the `EXPECTED_COMMANDS` array (after `"evolve"` or at the end, before the closing bracket):
...
**Step 2: Add agent-reference test for fix command**

After the `agentMap` in the "phase commands reference correct agents" test (around line 97-121), add an entry for `fix`:
...
Expected: FAIL — `fix.md` does not exist yet, so "all expected command files exist" and "fix.md should reference agent Luna" tests fail.

**Step 4: Commit**

```bash
git add test/commands.test.js
git commit -m "test(commands): add fix to expected commands and agent map"
```

---

### Task 2: Create the /rpi:fix command file

**Files:**
- Create: `commands/rpi/fix.md`

**Step 1: Write the command file**

Create `commands/rpi/fix.md` with the full command definition:
...
Run: `node --test test/commands.test.js`
Expected: All tests PASS — `fix.md` exists, has valid frontmatter, and references Luna, Mestre, Forge.

**Step 3: Commit**

```bash
git add commands/rpi/fix.md
git commit -m "feat(commands): add /rpi:fix quick bugfix command"
```

---

### Task 3: Update auto-flow detection for fix artifacts

**Files:**
- Modify: `commands/rpi/rpi.md:50-70`

**Step 1: Verify current auto-flow handles fix artifacts**

The auto-flow command (`/rpi`) detects phases by checking which artifacts exist. Since `/rpi:fix` creates the same artifacts (`REQUEST.md`, `plan/PLAN.md`, `implement/IMPLEMENT.md`), the auto-flow should already detect the state correctly:
- Has `REQUEST.md`, no `research/RESEARCH.md` → next = research
- But for fix features, there's no research — `plan/PLAN.md` exists directly.

The current detection logic (Step 3 of `rpi.md`) already handles this correctly:
- Step 2: Has `research/RESEARCH.md`, no `plan/PLAN.md` → next = plan
- Step 3: Has `plan/PLAN.md`, no `implement/IMPLEMENT.md` → next = implement

Since fix creates `plan/PLAN.md` without `research/RESEARCH.md`, the auto-flow would try to run research first. We need to make auto-flow skip research if `plan/PLAN.md` already exists.

**Step 2: Check if auto-flow already handles this**

Read `commands/rpi/rpi.md` Step 3 carefully. The detection is sequential:
1. Has `REQUEST.md`, no `research/RESEARCH.md` → next = research
2. Has `research/RESEARCH.md`, no `plan/PLAN.md` → next = plan

Step 1 would trigger for fix features (no RESEARCH.md). But Step 2 checks for RESEARCH.md which doesn't exist in fix. However, if `plan/PLAN.md` exists, it should skip to step 3.

The fix: change the detection to check `plan/PLAN.md` existence independently, not contingent on `research/RESEARCH.md`:

In `commands/rpi/rpi.md`, update Step 3 detection:
...
The key change is in step 1: adding `AND no plan/PLAN.md`. This way, if `/rpi:fix` created a PLAN.md without running research, the auto-flow skips research and goes to implement.

**Step 3: Run tests**

Run: `node --test test/commands.test.js`
Expected: PASS — the auto-flow test checks for phase names, not detection logic specifics.

**Step 4: Commit**

```bash
git add commands/rpi/rpi.md
git commit -m "fix(auto-flow): skip research if PLAN.md already exists"
```

---

### Task 4: Add fix-specific test

**Files:**
- Modify: `test/commands.test.js`

**Step 1: Add test for fix command specifics**

After the existing cross-reference tests, add:
...
```bash
git add test/commands.test.js
git commit -m "test(commands): add fix-specific content tests"
```
