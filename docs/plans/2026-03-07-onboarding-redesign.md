...
**Goal:** Replace `/rpi:onboarding` with a new command that auto-analyzes the user's codebase, generates a `.rpi-profile.md`, suggests features, and guides the user through their first feature interactively.

**Architecture:** Single command (`commands/rpi/onboarding.md`) orchestrates 5 phases: welcome, parallel codebase analysis (3 agents), profile generation, user priority selection, and guided first feature with 4 exit paths (build suggestion, describe own, demo, exit).

**Tech Stack:** Claude Code plugin commands (markdown frontmatter), Agent tool for parallel analysis, AskUserQuestion for interactive flow, Bash/Glob/Grep for codebase scanning.

---

### Task 1: Add `.rpi-profile.md` to `.gitignore`
...
Add `.rpi-profile.md` to `.gitignore` so generated profiles aren't committed to user projects.
...
```bash
git add .gitignore
git commit -m "chore: add .rpi-profile.md to .gitignore"
```

---

### Task 2: Rewrite `commands/rpi/onboarding.md` — Frontmatter + Phase 1

**Files:**
- Modify: `commands/rpi/onboarding.md`

**Step 1: Replace the entire file**

The new command needs `Agent` in `allowed-tools` (for the parallel analysis agents) and `Grep` (for agents scanning code). Replace the full content of `commands/rpi/onboarding.md` with the new command. Start with the frontmatter and Phase 1 (Welcome):
...
This establishes the new command structure. The full content is written in one shot (see Step 2 below for the complete file).

**Step 2: Write the complete command file**

The complete file content follows in the next steps (Tasks 2-6 build up the `<process>` sections). For implementation: write the ENTIRE file in one Write operation. The full content is specified across Tasks 2-6 — combine all sections into one file.

**Step 3: Commit**

```bash
git add commands/rpi/onboarding.md
git commit -m "feat(onboarding): rewrite command with codebase analysis and guided first feature"
```

---

### Task 3: Write Phase 2 — Codebase Analysis (3 Parallel Agents)

**Files:**
- Modify: `commands/rpi/onboarding.md` (continuation of Phase 2 section)

**Step 1: Add Phase 2 to the `<process>` block**

This section launches 3 agents in parallel using the Agent tool. Each agent has a focused scope and returns structured markdown.
...
**Step 2: No separate commit** — this is part of the same file as Task 2.

---

### Task 4: Write Phase 3 — Profile Generation

**Files:**
- Modify: `commands/rpi/onboarding.md` (continuation)

**Step 1: Add Phase 3 to `<process>`**
...
### Task 5: Write Phase 4 — User Priorities

**Files:**
- Modify: `commands/rpi/onboarding.md` (continuation)

**Step 1: Add Phase 4 to `<process>`**
...
### Task 6: Write Phase 5 — Guided First Feature

**Files:**
- Modify: `commands/rpi/onboarding.md` (continuation)

**Step 1: Add Phase 5 and close the `<process>` block**
...
**Step 2: Commit the complete file**

The entire `commands/rpi/onboarding.md` should be written in one operation combining all phases (Tasks 2-6). Then commit:

```bash
git add commands/rpi/onboarding.md
git commit -m "feat(onboarding): rewrite with codebase analysis, profile generation, and guided first feature"
```

---

### Task 7: Update `.gitignore` for `.rpi-profile.md`

**Files:**
- Modify: `.gitignore`

**Step 1: Add `.rpi-profile.md` to `.gitignore`**

Add this line after `.rpi.yaml`:

```
.rpi-profile.md
```

The full `.gitignore` becomes:
```
.DS_Store
node_modules/
*.log
...
```bash
git add .gitignore
git commit -m "chore: gitignore .rpi-profile.md"
```

---

### Task 8: Update `bin/onboarding.js` to reference new experience

**Files:**
- Modify: `bin/onboarding.js`

**Step 1: Update the "Get Started" step**

The terminal onboarding (`rpi-kit onboarding`) is a lightweight version for users who haven't installed the Claude Code plugin yet. Update the last step to mention the new in-editor onboarding:

In `bin/onboarding.js`, replace the `title: "Get Started"` step body with:
...
```bash
git add bin/onboarding.js
git commit -m "feat(onboarding): update terminal walkthrough to reference new in-editor onboarding"
```

---

### Task 9: Add test for `add-todo` in EXPECTED_COMMANDS (bug fix)
...
Add `"add-todo"` to the `EXPECTED_COMMANDS` array at line 9. Insert it after `"onboarding"`:
...
```bash
node --test test/commands.test.js 2>&1 | tail -10
```

Expected: `# pass 52` and `# fail 0`

**Step 3: Commit**

```bash
git add test/commands.test.js
git commit -m "fix(test): add add-todo to EXPECTED_COMMANDS"
```
...
```bash
node --test test/cli.test.js test/commands.test.js
```

Expected: All tests pass (9 CLI + 52 commands = 61 total, 0 failures).

**Step 2: Verify the new onboarding command has valid frontmatter**

The existing `commands.test.js` already checks:
- `onboarding.md` exists (line 20)
- Has valid frontmatter with `name: rpi:`, `description:`, `allowed-tools:` (lines 46-55)

The rewritten file must pass these checks.
...
|------|-------------|-------|
| 1 | Add `.rpi-profile.md` to `.gitignore` | `.gitignore` |
| 2 | Rewrite command — frontmatter + Phase 1 | `commands/rpi/onboarding.md` |
| 3 | Phase 2 — 3 parallel analysis agents | `commands/rpi/onboarding.md` |
| 4 | Phase 3 — profile generation | `commands/rpi/onboarding.md` |
| 5 | Phase 4 — user priorities | `commands/rpi/onboarding.md` |
| 6 | Phase 5 — guided first feature | `commands/rpi/onboarding.md` |
| 7 | `.gitignore` update | `.gitignore` |
| 8 | Update `bin/onboarding.js` | `bin/onboarding.js` |
| 9 | Fix `add-todo` test (bug) | `test/commands.test.js` |
| 10 | Run full test suite | — |

**Note:** Tasks 2-6 are all edits to the same file (`commands/rpi/onboarding.md`). In practice, write the complete file in one Write operation combining all phases, then commit once.
